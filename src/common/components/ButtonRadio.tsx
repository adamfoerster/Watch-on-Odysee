...
const getAttr = (x: string | SelectionOption, key: keyof SelectionOption): string =>
  typeof x === 'string' ? x : x[key]

export default function ButtonRadio<T extends string | SelectionOption = string>({
  name = 'buttonRadio',
  onChange,
  options,
  value,
  ...
  return (
    <div className="ButtonRadio">
      {options
        .map((o) => ({ o: getAttr(o, 'value'), display: getAttr(o, 'display') }))
        .map(({ o, display }) => (
          <div
            key={o}
            className={classnames('radio-button', { checked: value === o })}

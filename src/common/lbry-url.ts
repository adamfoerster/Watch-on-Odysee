...
const legacyClaim = (name: string, prefix = '') =>
  group(
    named(`${name}_name`, prefix + invalidNamesRegex) +
      oneOf(
        group('#' + named(`${name}_claim_id`, '[0-9a-f]{1,40}')),
...
export class PathSegment {
  constructor(public name: string, public claimID?: string, public sequence?: number, public amountOrder?: number) {}
...
function patternSegmenter(ptn: RegExp, url: string, options: UrlOptions = { encode: false }): string[] {
  ...
  const segments = match['channel_name']
    ? ['channel']
    : match['channel_with_stream_name']
    ? ['channel_with_stream', 'stream_in_channel']
    : match['stream_name']
    ? ['stream']
    : null
  ...
  return segments
    .map((s) => PathSegment.fromMatchGroup(s, match).toString())
